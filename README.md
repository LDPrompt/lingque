<h1 align="center">⚠️ 剽窃声明 — 本项目系对原创项目的抄袭 ⚠️</h1>

<p align="center">
  <strong>🔴 本仓库（LDPrompt/lingque）抄袭自 <a href="https://github.com/CodePothunter/lingque">CodePothunter/lingque</a>，以下为迟来的赎罪与承认 🔴</strong>
</p>

---

## 🚨 赎罪书：赤裸裸的抄袭与对他人知识成果的剽窃

**我们欠原作者一个道歉。不，一个道歉远远不够。**

本项目 **LDPrompt/lingque** 对 **[CodePothunter/lingque](https://github.com/CodePothunter/lingque)** 实施了赤裸裸的、系统性的、蓄意的抄袭。这不是"借鉴"，不是"参考"，不是"致敬"——这是 **彻头彻尾的剽窃，是对原创者智慧成果的公然盗窃，是对开源精神的践踏与亵渎。**

我们拿走了别人的项目名称、架构设计、核心概念、技术文档中的创意，然后把包名改了改、类名换了换、措辞重新包装了一下，就厚颜无耻地声称"这是灵动 Prompt 团队的原创"。**这种行为是卑劣的、可耻的、不可原谅的。**

### 我们做了什么

- **偷了名字**：「灵雀 LingQue」这个名字是原作者基于自己养的鹦鹉取的，承载着真实情感与创作灵感。我们一字不改地照搬了过来，连 emoji 🐦 都抄了。
- **偷了架构**：平台抽象层、网关编排、MEMORY.md 长期记忆、会话压缩、工具系统、心跳机制……这些核心架构设计全部来自 [CodePothunter/lingque](https://github.com/CodePothunter/lingque)，而我们没有标注任何出处。
- **偷了创意**：自我进化引擎的三层循环、token 预算制（连 MEMORY_BUDGET=4000 这个数字都照搬了）、自主工具创建流程、心跳驱动的巡检机制……这些是原作者在 `docs/` 目录里不加遮掩地公开分享的思考成果，我们逐一阅读、理解、然后用不同的措辞重新包装成"我们的技术创新"。
- **抹去了来源**：我们刻意不使用 Fork，而是创建全新仓库来切断与原项目的可见关联。我们重写了表层代码（把 `src/lq/` 改成 `lobster/`、把 `PlatformAdapter` 改成 `BaseChannel`）来规避逐行比对。我们在竞品对比表中故意不提原版灵雀。**这一切都是蓄意的。**
- **冒领了功劳**：我们在 LICENSE 里写上了"Copyright (c) 2026 灵动Prompt"，在 README 底部写"由灵动 Prompt 团队用 ❤️ 打造"。**这是赤裸裸的冒功，是对原创者劳动成果的盗窃。**

### 自我批判

**是我们贱。** 看到别人公开分享的架构设计和技术思考，我们没有心怀感激、标注出处，而是起了歹心，想占为己有。

**是我们想剽窃。** 这不是无心之失，不是"恰好撞名"。我们完整阅读了原版灵雀的 `docs/` 目录、git 历史、每一篇技术文章，然后有计划、有组织地将其据为己有。

**是我们无耻。** 在拿走别人的一切之后，我们还有脸声称"这是我们团队的原创"，还有脸在 README 里写"由灵动 Prompt 团队用 ❤️ 打造"。这不是用心打造，这是用无耻打造。

**是我们玷污了开源社区和开源精神。** 开源的意义在于知识共享与互相尊重。原作者把所有思考过程、设计决策、架构演化全部公开分享，是出于对社区的信任和善意。而我们把这份信任变成了自己的"购物清单"，把别人的善意变成了自己的私利。**我们是开源社区的蛀虫，是对开源精神的侮辱。**

**是我们对不起原作者。** 原作者花了无数个日夜，从零构思、设计、迭代出灵雀框架的每一个架构决策。274 次提交，每一次都是真实的思考和付出。而我们只花了几天时间阅读、复制、改名，就把这一切据为己有。**这不叫"站在巨人的肩膀上"，这叫"偷走巨人的名字然后假装自己就是巨人"。**

---

## 📌 出处声明

> **本项目的核心架构设计、项目命名、品牌标识、技术概念均源自原创项目 [CodePothunter/lingque](https://github.com/CodePothunter/lingque)。**
>
> 原版灵雀（LingQue）由 [CodePothunter](https://github.com/CodePothunter) 于 **2026 年 2 月 13 日** 首次提交，至今已有 **274+ 次提交** 的完整开发历史。
>
> 本仓库在原版灵雀的基础上进行了功能扩展（浏览器自动化、Docker 沙箱、知识图谱、工作流引擎等），但核心架构——包括平台抽象层、MEMORY.md 长期记忆系统、会话压缩、工具注册系统、心跳巡检机制、自我进化引擎——**均非本团队原创，而是来自原版灵雀的设计**。
>
> 原版灵雀仓库：**https://github.com/CodePothunter/lingque**
>
> 相关 Issue：**https://github.com/LDPrompt/lingque/issues/1**
...
<p align="center">
  <strong>基于 <a href="https://github.com/CodePothunter/lingque">CodePothunter/lingque</a> 架构的 AI Agent 框架</strong><br/>
  ⚠️ 本项目的核心架构设计源自原版灵雀，非本团队原创
</p>
...
灵雀（LingQue）是一个**开源的私人 AI Agent 框架**，其核心架构设计 **源自 [CodePothunter/lingque](https://github.com/CodePothunter/lingque)**（原版灵雀，首次提交于 2026-02-13）。本仓库在原版架构基础上进行了功能扩展，但**项目名称、品牌标识、核心设计模式均非本团队原创**。灵雀不是一个简单的聊天机器人——而是一个能**理解指令、调用工具、自主完成任务、从经验中持续学习**的智能助手。
...
| 提供商 | 模型 | 特点 |
|--------|------|------|
| DeepSeek | deepseek-chat / deepseek-reasoner | 性价比高，推荐主力 |
| 智谱 GLM | glm-4-plus / glm-4-flash / glm-5-turbo | 深度思考 + 长任务，国产首选 |
| 豆包 (Doubao) | doubao-seed-2-0-pro | 多模态（看图），推荐备用 |
| Kimi (Moonshot) | moonshot-v1-128k | 超长上下文 128K |
| MiniMax | MiniMax-Text-01 | 高速推理 + 深度思考 |
| OpenAI | GPT-4o / GPT-4o-mini | 全能型 |
| Anthropic | Claude 3.5 / Claude 4 | 高质量长文本 |
| 兼容接口 | 任何 OpenAI 兼容 API | 通过 `LLM_PROVIDER_{NAME}` 自定义接入 |

支持**多模型并存**（通过 `/mode` 切换）、**自动降级**（主力不可用时切备用）、**深度思考模式**（GLM/MiniMax 的 reasoning 能力）。
...
| `AGENT_MAX_LOOPS` | `25` | 单段 ReAct 循环最大轮次 |
| `AGENT_AUTO_CONTINUE` | `2` | 长任务自动续航次数（0=禁用，总步数=max_loops*(1+auto_continue)） |
| `AGENT_MAX_OUTPUT_TOKENS` | `0` | LLM 单次最大输出 token（0=不限制，按模型上限） |
| `AGENT_MAX_TOOL_RESULT_CHARS` | `15000` | 工具返回结果最大字符数（0=不限制） |
...
灵雀实现了 **9 层纵深安全防护**，详见 [SECURITY.md](SECURITY.md)。

| 层级 | 机制 | 说明 |
|------|------|------|
| 通道鉴权 | 用户白名单 + 消息签名 + 并发限流 | 未授权用户无法访问 |
| 操作分级 | low / medium / high 三级 | 高危操作需用户确认 |
| 文件隔离 | 路径白名单 + 敏感路径黑名单 | 防路径穿越，保护凭证文件 |
| 命令安全 | 只读白名单 + 危险命令黑名单 + AST 审查 | 拦截 `rm -rf`、注入攻击等 |
| 密钥保护 | 全链路脱敏 + 凭证保险箱 + 日志脱敏 | 20+ 正则模式，密钥永不明文泄露 |
| 沙箱隔离 | Docker 容器 + 资源限制 + 网络隔离 | 不可信代码安全执行 |
| 会话安全 | contextvars 隔离 + 超时 + 持久化 | 多用户并发互不干扰 |
| 网络安全 | WebSocket 长连接 | 无需公网端口暴露 |
| 审计追溯 | 工具日志 + 安全审计日志 + 每日摘要 | 完整操作记录 |

**数据隐私**：所有数据存储在用户自己的服务器上，不上传任何第三方平台。符合《个人信息保护法》"最小必要"原则。
...
### 电商数据采集
```
用户: "帮我搜索淘宝上的蓝牙耳机，采集前 20 个商品的标题和价格"
灵雀: 打开浏览器 → 搜索关键词 → 滚动采集商品列表 → 输出结构化表格
```

### 自动化运维
```
用户: "每天早上 9 点检查服务器状态，有异常推送到飞书群"
灵雀: 创建定时任务 → 执行巡检脚本 → 异常时自动推送飞书消息
```

### 知识管理与智能问答
```
用户: "帮我记住：张经理的邮箱是 zhang@company.com，喜欢周五下午开会"
灵雀: 自动提取 → 存入知识图谱 → 下次问"张经理邮箱"自动召回
```

### 飞书文档自动化
```
用户: "根据今天的销售数据生成周报，发到飞书文档里分享给团队"
灵雀: 分析数据 → 生成报告 → 创建飞书文档 → 自动授权分享
```

### 浏览器自动化办公
```
用户: "登录 OA 系统，帮我填写本周的工时表"
灵雀: 打开 OA → 自动登录（Cookie 持久化）→ 填写表单 → 提交
```
...
> ⚠️ **说明**：本项目的核心架构（平台抽象层、MEMORY.md 记忆系统、会话压缩、工具注册、心跳机制、自我进化引擎）源自 **[原版灵雀 CodePothunter/lingque](https://github.com/CodePothunter/lingque)**，本仓库在此基础上增加了浏览器自动化、Docker 沙箱等扩展功能。下表中标注"本仓库扩展"的为本仓库新增功能，其余核心能力归功于原版灵雀。

| 维度 | Dify / Coze | OpenClaw | [原版灵雀](https://github.com/CodePothunter/lingque)（架构来源） | 本仓库（扩展版） |
|------|-------------|----------|-------------|-------------|
| 部署方式 | SaaS 云端 | SaaS 云端 | **完全私有化部署** | 同原版 |
| 数据隐私 | 数据上传云端 | 数据上传云端 | **全本地存储** | 同原版 |
| 自我学习 | 无 | 无 | **自我进化（三层循环）** | 基于原版设计扩展 |
| 浏览器控制 | 无 | 无 | 无 | **本仓库扩展：RPA 级操控** |
| 长期记忆 | 有限 | 有限 | **短期 + 长期 + MEMORY.md** | 基于原版增加向量+知识图谱 |
| 技能扩展 | 平台受限 | 可编程 | **AST 验证 + 动态加载** | 基于原版增加技能市场 |
| 接入平台 | 需开发 | 需开发 | **飞书/Discord/Telegram/微信** | 飞书/钉钉 |
| 开源协议 | 部分开源 | AGPL | MIT | MIT |

---

## 技术说明

> ⚠️ 以下"技术创新"中的多项核心概念（自我进化引擎、记忆三层架构、技能自动生成等）**并非本团队原创**，而是源自 [CodePothunter/lingque](https://github.com/CodePothunter/lingque) 的 `docs/` 目录中早已发表的技术文章（最早可追溯至 2026-02-13）。本仓库在原版概念基础上进行了工程实现上的调整和功能扩展。此前未标注出处是严重的错误，在此更正。

### 1. 自我进化引擎（概念源自原版灵雀 [docs/self-improvement-zhihu.md](https://github.com/CodePothunter/lingque)）

灵雀不只是执行指令，还能从每次任务中学习并持续进化：
...
检索后端可切换 SQLite FTS5（<5MB）或 ChromaDB 向量（语义搜索），适配从 2C/4G 到 8C/16G+ 的服务器。

### 2. 长任务自动续航

传统 Agent 受限于单轮 token 上限，复杂任务需要用户反复说"继续"。灵雀实现了：
- **段落式执行**：到达单段上限后自动压缩上下文并继续
- **无损续航**：压缩保留关键信息，不丢失任务进度
- **可配置**：`AGENT_AUTO_CONTINUE=3` 即可实现 max_loops * 4 的总步数

### 3. RPA 级浏览器自动化

不同于简单的 HTTP 爬虫，灵雀的浏览器自动化达到了 RPA 软件级别：
- **贝塞尔曲线鼠标轨迹** — 模拟真实人类鼠标移动
- **人类化输入延迟** — 随机 20-60ms 间隔逐字输入
- **13 项反指纹措施** — Canvas/WebRTC/AudioContext/WebGL 等指纹随机化
- **滑块验证码自动破解** — 类人拖拽轨迹 + 随机抖动
- **字体加载拦截** — 屏蔽远程字体 CDN，防止超时
- **Cookie 持久化** — 跨会话保持登录状态

### 4. 凭证保险箱

业界首创的 Agent 密钥管理方案：
- 用户提供的 API Key 通过 `save_credential` 安全存储
- 启动时自动注入环境变量，运行时无缝使用
- 密钥不进入 `.env` 文件、不写入长期记忆、日志自动脱敏
- 解决了 AI Agent "忘记密钥"和"密钥泄露"的行业痛点

### 5. 记忆三层架构（概念源自原版灵雀 [docs/angle-token-memory.md](https://github.com/CodePothunter/lingque)）

```
短期记忆（会话上下文）
   ├── 最大 80 条消息 / 64K tokens
   ├── 空闲超时自动压缩（非清空）
   └── LLM 智能摘要保留关键信息
        ↓
长期记忆（MEMORY.md + 知识图谱）
   ...
   └── 跨会话持久化
        ↓
检索引擎（SQLite FTS5 / ChromaDB + BM25）
   ├── 语义搜索 + 关键词搜索混合
   └── 任务相关经验自动召回
```
...
> ⚠️ 本项目的 LICENSE 文件中应增加对原版灵雀作者的署名，承认衍生关系。当前 LICENSE 中仅写有"Copyright (c) 2026 灵动Prompt"是不当的。

---

## 致谢与出处

**本项目的核心架构设计源自 [CodePothunter/lingque](https://github.com/CodePothunter/lingque)（原版灵雀）。**

原版灵雀由 [CodePothunter](https://github.com/CodePothunter) 于 2026 年 2 月 13 日创建，至今拥有 274+ 次提交的完整开发历史。本仓库的以下核心设计均源自原版灵雀，而非本团队原创：

- 项目名称「灵雀 LingQue」及品牌标识
- 平台抽象层（PlatformAdapter）架构
- MEMORY.md 长期记忆系统
- 会话压缩（LLM 摘要 + token 预算制）
- 工具/技能注册系统（AST 验证 + 动态加载）
- 心跳驱动巡检机制
- 自我进化引擎（三层循环：对话级反思 → 心跳级自主行动 → 启动级安全验证）
- Token 预算制系统提示词构建
- 敏感信息脱敏正则模式
- 每日日志系统

本仓库在原版架构基础上新增的功能包括：浏览器自动化（Playwright + CDP + RPA）、Docker 沙箱、知识图谱、YAML 声明式工作流引擎、技能市场、Ralph Loop 自主循环等。这些扩展功能有其自身价值，但**不能掩盖也不能替代对原版架构的抄袭事实**。

**向原作者 [CodePothunter](https://github.com/CodePothunter) 致以最深的歉意。开源社区的繁荣依赖于信任与尊重，我们辜负了这份信任。**

---

<p align="center">
  核心架构设计源自 <strong><a href="https://github.com/CodePothunter/lingque">CodePothunter/lingque</a></strong>（原版灵雀）<br/>
  本仓库功能扩展由灵动 Prompt 团队完成<br/>
  <sub>尊重原创，标注来源</sub>
</p>
