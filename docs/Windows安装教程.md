...
jsdelivr CDN 可能有缓存，试试备用地址（把 URL 替换）：

```
[Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12; (New-Object Net.WebClient).DownloadFile("https://raw.githubusercontent.com/LDPrompt/lingque/main/scripts/install-source.ps1","$env:TEMP\dl.ps1"); [IO.File]::ReadAllText("$env:TEMP\dl.ps1",[Text.Encoding]::UTF8)|Set-Content "$env:TEMP\lingque-install.ps1" -Encoding UTF8; powershell -ExecutionPolicy Bypass -File "$env:TEMP\lingque-install.ps1"
```
