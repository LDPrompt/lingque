...
在打开的蓝色窗口里，**复制** 下面这一整行（很长，一定要复制完整），然后在窗口里 **右键粘贴**，按 **回车**：
...
```
Invoke-WebRequest "https://raw.githubusercontent.com/LDPrompt/lingque/main/scripts/install-source.ps1" -OutFile "$env:TEMP\lingque-install.ps1"; & "$env:TEMP\lingque-install.ps1"
```
...
```
powershell -ExecutionPolicy Bypass -Command "Invoke-WebRequest 'https://cdn.jsdelivr.net/gh/LDPrompt/lingque@main/scripts/install-source.ps1' -OutFile '$env:TEMP\lingque-install.ps1'; & '$env:TEMP\lingque-install.ps1'"
```
